...
func lookProxy(lookup Proxy, c *gin.Context) {
	...
	proxy.Director = func(req *http.Request) {
		...
		}() + func () string {if len(lookup.AccessPostfix) <= 1 {return lookup.AccessPostfix} else if path[len(path)-1] != '/'{return lookup.AccessPostfix} else {return lookup.AccessPostfix[1:]}}() + path + func() string {
